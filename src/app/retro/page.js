...
export default function CamisasRetro() {
  ...
  const fetchCamisas = useCallback(async (page = 1, search = '') => {
    setLoading(true);
    try {
      let url = `/api/camisas?categorias=RETRO&page=${page}&limit=50`;
      if (search.trim()) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/" className="mt-4 inline-block bg-purple-600 text-white px-4 py-2 rounded-md hover:bg-purple-700">
            Volver al inicio
          </Link>
        </div>
      </div>
